...
MobileNet基于深度可分卷积，深度可分卷积是一种分解卷积，把标准卷积分解为深度卷积和1\*1卷积（称为点卷积）。MobileNes的深度卷积是把单个滤波器用于每个输入通道上。点卷积然后用1\*1滤波器把深度卷积的结果结合起来。深度可分卷积分为两层，一个卷积一个结合。这个分解极大的减少了计算量和模型大小。图2展示了一个标准卷积分解为深度卷积和1\*1点卷积。
...
先是深度可分卷积与完全卷积的结果。见表4，使用深度可分卷积比全卷积只减少了1%的准确率，计算量和参数数量得到了减少。
...
表6显示了准确率，计算量和大小关于宽度乘子的权衡。

表7显示了准确率，计算量和大小关于分辨率乘子的权衡。

![mobilenet-tab-6-7.png](R/mobilenet-tab-6-7.png)

图4显示了$\alpha$为1，0.75，0.5，0.25和分辨率为224，192，160，128的16个模型在ImageNet的准确率和计算量的权衡。
...
在斯坦福狗数据集上进行了训练，可以达到最好的结果，计算量和大小都极大的减小。
