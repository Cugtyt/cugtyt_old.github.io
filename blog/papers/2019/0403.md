...
由Sutton可知，当没有中间奖励的时候，生成模型（策略）$G_{\theta}\left(y_{t} | Y_{1 : t-1}\right)$的目标是从一个起始状态$s_0$生成最大化期望奖励的序列：

$$J(\theta)=\mathbb{E}\left[R_{T} | s_{0}, \theta\right]=\sum_{y_{1} \in \mathcal{Y}} G_{\theta}\left(y_{1} | s_{0}\right) \cdot Q_{D_{\phi}}^{G_{\theta}}\left(s_{0}, y_{1}\right), \qquad (1)$$
