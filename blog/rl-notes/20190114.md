...
$$J ( \pi ) = \int _ { \tau } P ( \tau | \pi ) R ( \tau ) = \underset { \tau \sim \pi } { \mathrm { E } } [ R ( \tau ) ]$$
...
其中$s'\sim P$是$s'\sim P(\cdot | s，a)$的简写，表示从环境转换规则中采样的下一个状态$s'$; $a \sim \pi$是$\sim \pi(\cdot | s)$的简写; 而$a'\sim \pi$是$a'\sim \pi(\cdot | s')$的简写。
...
对于策略$\pi$的优势函数$A ^ {\pi}(s，a)$描述了在状态s中采取特定动作a比根据$\pi(\cdot | s)$随机选择动作要好多少,这里假设一直按照$\pi$采取行动。在数学上优势函数定义为：
